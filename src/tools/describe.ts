import { McpServer } from "@modelcontextprotocol/sdk/server/mcp.js";
import {
  describeSObject,
  describeGlobal,
  getRecord,
} from "../services/salesforce.js";
import {
  toJson,
  fieldsToMarkdownTable,
  truncate,
} from "../services/formatting.js";
import {
  DescribeObjectSchema,
  ListObjectsSchema,
  GetRecordSchema,
} from "../schemas/index.js";
import { getAuthContext } from "../auth-context.js";
import { McpToolResponse } from "../types.js";

export function registerDescribeTools(server: McpServer): void {
  server.registerTool(
    "sf_list_objects",
    {
      title: "List Salesforce Objects",
      description: `List all SObjects available in the connected user's org.

Args:
  - queryable_only (default true): Filter to queryable objects
  - filter (optional): Substring filter on name/label

Examples:
  - "What objects are available?" → queryable_only: true
  - "Find custom objects" → filter: "__c"`,
      inputSchema: ListObjectsSchema,
      annotations: {
        readOnlyHint: true,
        destructiveHint: false,
        idempotentHint: true,
        openWorldHint: true,
      },
    },
    async ({ queryable_only, filter }): Promise<McpToolResponse> => {
      const auth = getAuthContext();
      const global = await describeGlobal(auth);
      let objects = global.sobjects;
      if (queryable_only) objects = objects.filter((o) => o.queryable);
      if (filter) {
        const lc = filter.toLowerCase();
        objects = objects.filter(
          (o) =>
            o.name.toLowerCase().includes(lc) ||
            o.label.toLowerCase().includes(lc)
        );
      }
      const header = `**Total objects:** ${objects.length}`;
      const table = [
        "| Name | Label | Queryable | Createable | Updateable |",
        "| --- | --- | --- | --- | --- |",
        ...objects.map(
          (o) =>
            `| ${o.name} | ${o.label} | ${o.queryable ? "✅" : "❌"} | ${
              o.createable ? "✅" : "❌"
            } | ${o.updateable ? "✅" : "❌"} |`
        ),
      ].join("\n");
      return {
        content: [{ type: "text", text: truncate(`${header}\n\n${table}`) }],
        structuredContent: { total: objects.length, objects } as unknown as Record<string, unknown>,
      };
    }
  );

  server.registerTool(
    "sf_describe_object",
    {
      title: "Describe Salesforce Object Schema",
      description: `Get the full schema for a Salesforce SObject – fields, types, and permissions.

Args:
  - object_name (required): e.g. "Account", "Case", "MyObject__c"
  - include_fields (default true)

Examples:
  - "What fields does Case have?" → object_name: "Case"`,
      inputSchema: DescribeObjectSchema,
      annotations: {
        readOnlyHint: true,
        destructiveHint: false,
        idempotentHint: true,
        openWorldHint: true,
      },
    },
    async ({ object_name, include_fields }): Promise<McpToolResponse> => {
      const auth = getAuthContext();
      const describe = await describeSObject(auth, object_name);
      const meta = [
        `## ${describe.label} (\`${describe.name}\`)`,
        `**Label Plural:** ${describe.labelPlural}`,
        `**Key Prefix:** ${describe.keyPrefix ?? "N/A"}`,
        `**Queryable:** ${describe.queryable ? "✅" : "❌"}  |  **Createable:** ${
          describe.createable ? "✅" : "❌"
        }  |  **Updateable:** ${describe.updateable ? "✅" : "❌"}`,
        `**Total Fields:** ${describe.fields.length}`,
      ].join("\n");
      const fieldsMarkdown = include_fields
        ? `\n\n### Fields\n${fieldsToMarkdownTable(describe.fields)}`
        : "";
      return {
        content: [{ type: "text", text: truncate(meta + fieldsMarkdown) }],
        structuredContent: {
          name: describe.name,
          label: describe.label,
          fieldCount: describe.fields.length,
        },
      };
    }
  );

  server.registerTool(
    "sf_get_record",
    {
      title: "Get Salesforce Record by ID",
      description: `Retrieve a single record by its ID.

Args:
  - object_name (required): e.g. "Account", "Case"
  - record_id (required): 15 or 18-char Salesforce ID
  - fields (optional): Specific fields. Omit for all readable.
  - response_format ("markdown" | "json", default "markdown")`,
      inputSchema: GetRecordSchema,
      annotations: {
        readOnlyHint: true,
        destructiveHint: false,
        idempotentHint: true,
        openWorldHint: true,
      },
    },
    async ({ object_name, record_id, fields, response_format }): Promise<McpToolResponse> => {
      const auth = getAuthContext();
      const record = await getRecord(auth, object_name, record_id, fields);
      if (response_format === "json") {
        return {
          content: [{ type: "text", text: toJson(record) }],
          structuredContent: record,
        };
      }
      const lines = Object.entries(record)
        .filter(([k]) => k !== "attributes")
        .map(([k, v]) => `**${k}:** ${v === null || v === undefined ? "_null_" : String(v)}`);
      return {
        content: [
          {
            type: "text",
            text: truncate(`## ${object_name} Record: \`${record_id}\`\n\n${lines.join("\n")}`),
          },
        ],
        structuredContent: record,
      };
    }
  );
}
